#!/usr/bin/env bash
echo "====================ROUTE53==========="
ROUTE53=$(aws route53 list-hosted-zones | jq '.HostedZones[] | {Name: .Name, Config: .Config.PrivateZone, ResourceRecordSetCount: .ResourceRecordSetCount } | join("#") ' | tr -d '[]," '| tr -d '\r')
for item in $ROUTE53
do
